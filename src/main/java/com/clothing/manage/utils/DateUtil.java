...
//        String st2r = "1539347124561";
//        String s = stampToDate(st2r);
//        System.out.println(s);
//        Date date = strToDate("2018-10-05 00:00:17", "yyyy-MM-dd HH:mm:ss");
//        Double orderBill2 = getOrderBill(date, new Date(), 5, 2D, 20D);
//        System.out.println("orderBill2:"+orderBill2);

//        getOrderBill(DateUtil.strToDate("2018-09-13 16:43:14", "yyyy-MM-dd HH:mm:ss"), new Date(),);
        Double orderBill1 = getOrderBill(1, 705, 2D, 15D);
        System.out.println(orderBill1);
        if (true) {
            return;
        }
...
//        System.out.println("shop="+String.format("%.1f",shop));
    ...
    public static Double getOrderBill(Integer freeUseTime, Integer useMinute, Double unitPrice, Double highCost) {
        try {
            // 计费使用时长
            Integer useMinuteMoney = useMinute - freeUseTime;
            if (useMinuteMoney <= 0) {
                return 0D;
            }
            /** 分钟转化为小时单位，并不满一个小时的按一个小时计算收费
             *  2018-10-12 15:17
             *  修改为按自然天计算订单费用
             */
            Integer useHour = useMinuteMoney % 60 != 0 ? useMinuteMoney / 60 + 1 : useMinuteMoney / 60;
            Integer dayNum = useHour / 24;
            BigDecimal countBill = new BigDecimal(0);
            if (dayNum > 0) {
                if(highCost>(unitPrice * 24)){
                    highCost = unitPrice * 24;
                }
                countBill = (new BigDecimal(dayNum)).multiply(new BigDecimal(highCost));
                useHour = useHour % 24;
            }
            BigDecimal lastDayBill = (new BigDecimal(useHour)).multiply(new BigDecimal(unitPrice));
            if (lastDayBill.doubleValue() > highCost) {
                lastDayBill = new BigDecimal(highCost);
            }
            Double bill = (countBill.add(lastDayBill)).doubleValue();
            return bill;
        } catch (Exception e) {
            e.printStackTrace();
            return 0D;
